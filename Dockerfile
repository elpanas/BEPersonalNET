...
WORKDIR /src
COPY ["BEPersonal.csproj", "."]
RUN dotnet restore "./BEPersonal.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./BEPersonal.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./BEPersonal.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "BEPersonal.dll"]